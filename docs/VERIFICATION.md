...
* **Runtime is partially refined (M11 Phase A.7 ongoing).** Phase A.6
  ...
  invariant + `runtime_forward_order_preserved` corollary as the
  foundation for runtime ref-FIFO-e2e (the `forwardedAt` ledger
  is faithful — recorded indices point at the correct msg on the
  forwarder's outgoing channel; combined with `InvDeliverIsPrefix`
  at the b→c channel, this gives msgs forwarded at trace-indices
  i₁ < i₂ arrive at c in that same index order). **Still pending**:
  packaging this as a single named `runtime_ref_fifo_e2e` theorem
  over `Reachable`. A behavioural connection from `Captp.Session.run`
