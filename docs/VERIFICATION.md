...
* **Runtime is partially refined (M11 Phase A.7 ongoing).** Phase A.6
  ...
  Network satisfies `Spec.Valid`); the `InvForwardedAgreesWithSent`
  invariant + `runtime_forward_order_preserved` + `runtime_ref_fifo_e2e`
  (the full headline: msgs forwarded earlier arrive at the resolution
  host earlier — runtime analog of Phase A.5's `ref_fifo_e2e`).
  **Still pending**: a behavioural connection from `Captp.Session.run`
  to the LTS step-by-step. Session.run calls `Netlayer.sendMessage`
  / `recvMessage?`, which when backed by `Network.netlayerFor` go
  through `Network.send` / `recv?`. The per-op correspondence
  proven in Phase A.7-γ ties those to LTS steps; packaging this as
  a refinement theorem at the Session.run level is the remaining
  work.
