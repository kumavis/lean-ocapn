...
plus per-ordered-pair `ChannelState` (the wire between two vats). -/
...
configuration, every channel empty. -/
...
to `dst`. Fails (returns `none`) if `msg` has already appeared on this
channel. Per-channel msg-uniqueness mirrors the Veil precondition
`∀ K, ¬ sentAt s d m K`. -/
